...
function extractMensIccRows14To34(rows: string[][], fallbackHeader: string[]): IccTable {
  const start = 13; // row 14 (1-based)
  const end = 33; // row 34 (1-based)
  const segment = rows.slice(start, end + 1);

  const headerIdxInSegment = segment.findIndex((r) => {
    const normalized = r.map((c) => cleanMojibake(c || "").toLowerCase());
    return normalized.some((c) => c.includes("tournament")) && normalized.some((c) => c.includes("winner"));
  });

  if (headerIdxInSegment === -1) return { header: fallbackHeader, body: [] };

  const headerRow = segment[headerIdxInSegment].map((c) => cleanMojibake(c || ""));
                    ...
                    {row.map((cell, cIdx) => (
                      <td
                        ...
                        className={`border-t border-amber-200/20 px-4 py-3 align-top text-amber-50/95 ${
                          cIdx === 0 || cIdx === 3 ? "whitespace-normal break-words" : "whitespace-nowrap"
                        }`}
                      >
                        {cleanMojibake(cell || "") || "-"}
                      </td>
