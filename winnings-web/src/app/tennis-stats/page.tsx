...
export default function TennisStatsPage() {
  ...
  const header = useMemo(() => (rows.length > 0 ? rows[0] : []), [rows]);
    ...
    <div className="relative min-h-screen overflow-hidden text-white">
      ...
      <main className="relative z-10 mx-auto w-full max-w-6xl px-4 py-8 sm:px-8 sm:py-10">
        <section className="mb-6 rounded-3xl border border-white/20 bg-white/10 p-6 shadow-2xl backdrop-blur-xl sm:p-8">
          ...
          <p className="mt-3 max-w-3xl text-white/80">Select a tournament sheet to display its exact Excel data.</p>
        ...
        <div className="mb-4 rounded-xl border border-cyan-300/30 bg-cyan-300/10 px-4 py-3 text-sm">
          Active sheet: <span className="font-semibold text-cyan-200">{selectedSheet}</span>
        </div>

        {loading ? <div className="mb-4 text-sm text-cyan-100">Loading sheet data...</div> : null}
        {error ? <div className="mb-4 text-sm text-rose-300">Error: {error}</div> : null}
          ...
          <table className="min-w-full text-left text-sm">
            {header.length > 0 ? (
              <thead className="bg-white/15 text-cyan-100">
                <tr>
                  {header.map((h, idx) => (
                    <th key={`${h}-${idx}`} className="px-4 py-3 whitespace-nowrap">
                      {h || `Column ${idx + 1}`}
                    </th>
                  ))}
                </tr>
