...
type SoccerCategory = "FIFA" | "Continental";
...
function sectionBounds(rows: string[][], category: SoccerCategory) {
  const start = rows.findIndex((r) => clean(r[0] || "").toLowerCase() === category.toLowerCase());
  if (start === -1) return null;

  let end = rows.length;
  for (let i = start + 1; i < rows.length; i++) {
    const first = clean(rows[i][0] || "").toLowerCase();
    if (first === "fifa" || first === "continental") {
      end = i;
      break;
    }
  }
...
export default function SoccerStatsPage() {
  ...
  const [selectedCategory, setSelectedCategory] = useState<SoccerCategory>("FIFA");
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      setError(null);
      try {
        const res = await fetch(`/api/sheet-data?sheet=${encodeURIComponent("Soccer")}`, { cache: "no-store" });
        const data = await res.json();
        if (!res.ok) throw new Error(data?.error || "Failed to fetch soccer data");
  ...
  const table = useMemo(() => {
    ...
    const headerRow = section.find((r) => clean(r[1] || "").toLowerCase() === "position") || [];

    const nonEmptyIndices = headerRow
      .map((c, i) => ({ c: clean(c || ""), i }))
      .filter(({ c, i }) => i > 0 && c)
      .map(({ i }) => i);

    const colCount = nonEmptyIndices.length ? Math.max(...nonEmptyIndices) + 1 : 0;

    const header = headerRow
      .slice(1, colCount)
      .map((c, i) => clean(c || "") || `Column ${i + 1}`);

    const body = section
      .slice(section.indexOf(headerRow) + 1)
      .map((r) => r.slice(1, colCount).map((c) => clean(c || "")))
      .filter((r) => clean(r[0] || "") !== "")
      .map((r) => r.map((c, idx) => (idx === 0 ? c : c || "-")));
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,_#123026_0%,_#0b1627_45%,_#05070f_100%)] px-3 py-6 text-[#F5E6B3] sm:px-6 sm:py-8 lg:px-8 lg:py-10">
      <main className="relative z-30 mx-auto w-full max-w-6xl rounded-2xl border border-amber-300/30 bg-black/55 p-4 shadow-[0_0_60px_rgba(245,185,59,0.12)] backdrop-blur-xl sm:rounded-3xl sm:p-8">
        <section className="relative mt-2 mb-4 overflow-visible rounded-2xl border border-amber-200/25 bg-black/45 p-4 sm:p-6">
          <div aria-hidden="true" className="pointer-events-none absolute inset-0 bg-cover bg-center opacity-35" style={{ backgroundImage: "url('/soccer-aura-max-2026.svg')" }} />
          ...
          <h1 className="relative z-0 mt-3 break-words text-[clamp(1.5rem,6vw,2rem)] font-bold leading-tight text-amber-100 sm:z-10 sm:text-4xl">Soccer Winnings</h1>
        </section>
        ...
        <section className="mt-5">
          <h2 className="mb-3 break-words text-sm font-semibold leading-tight text-amber-100/90">Soccer Categories</h2>
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
            {(["FIFA", "Continental"] as const).map((cat) => (
              <button
                key={cat}
            ...
            <table className="min-w-full table-fixed border-separate border-spacing-0 border border-amber-200/35 text-left text-sm">
                  ...
                  {table.header.map((cell, idx) => (
                    <th
                      ...
                      className={`border-y border-amber-200/35 px-4 py-3 text-xs font-semibold tracking-wide sm:text-sm ${
                        idx === 0
                          ? "sticky left-0 z-20 whitespace-normal break-words bg-[#122b24]"
              ...
              <tbody>
                {table.body.map((row, rIdx) => (
                  <tr key={`soccer-row-${rIdx}`} className="odd:bg-black/25 even:bg-black/45">
                    {row.map((cell, cIdx) => (
                      <td
                        key={`soccer-${rIdx}-${cIdx}`}
                        className={`border-t border-amber-200/20 px-4 py-3 align-top text-amber-50/95 ${
                          cIdx === 0
                            ? "sticky left-0 z-10 whitespace-normal break-words bg-[#0f211c]"
                            : "whitespace-nowrap text-center"
                        }`}
                      >
                        {cell || "-"}
              ...
              {chartData.map((d, i) => {
                ...
                return (
                  <g key={`${d.label}-${i}`}>
                    ...
                    <text x={x} y={Math.max(12, y - 10)} textAnchor="middle" fontSize="10" fill="rgba(253,230,138,0.95)">
                      {d.label}
                    </text>
                  </g>
                );
